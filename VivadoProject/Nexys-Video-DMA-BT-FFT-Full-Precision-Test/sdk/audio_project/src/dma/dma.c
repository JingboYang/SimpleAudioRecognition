...
XStatus fnConfigDmaFFT(XAxiDma *AxiDma)
{
	int Status;
	XAxiDma_Config *pCfgPtr;
...
XStatus fnConfigDmaLOG(XAxiDma *AxiDma)
{
	int Status;
	XAxiDma_Config *pCfgPtr;
	...
	//Make sure the DMA hardware is present in the project
	//Ensures that the DMA hardware has been loaded
	pCfgPtr = XAxiDma_LookupConfig(XPAR_AXIDMA_2_DEVICE_ID);
	if (!pCfgPtr)
	{
		if (Demo.u8Verbose)
		{
			xil_printf("\r\nNo config found for %d", XPAR_AXIDMA_2_DEVICE_ID);
		}
		return XST_FAILURE;
	}
